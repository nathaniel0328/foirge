...
local function SmartBust(targetSpawner, atmModel)
    ...
    pcall(function()
        ...
        -- Adding random offset to start delay to bypass macro detection
        local randomStartOffset = math.random(-5, 5) / 100
        task.wait(math.max(0.05, Config.task3 + randomStartOffset))
        ...
        -- Adding random offset to cooldown
        local randomWaitOffset = math.random(-20, 20) / 100
        task.wait(math.max(0.5, Config.task4 + randomWaitOffset))
        ...
        -- Adding random offset to receive delay
        local randomEndOffset = math.random(-5, 5) / 100
        task.wait(math.max(0.05, Config.task5 + randomEndOffset))
